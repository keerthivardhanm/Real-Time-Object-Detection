...
=========================================
A production-ready, full-featured object detection app using YOLOv8.
...
def webcam_detection_tab(model, confidence, iou_threshold):
    ...
    if not camera_available:
        st.error("❌ No webcam detected. Please connect a camera.")
        return
        ...
        try:
            while run_webcam:
                ...
                <div class='stats-card'>
                    ...
                    <p><strong>Objects:</strong> {sum(class_counts.values())}</p>
                    <p><strong>Classes:</strong> {', '.join(class_counts.keys())}</p>
                </div>
                """, unsafe_allow_html=True)
...
def video_detection_tab(model, confidence, iou_threshold):
    ...
    if uploaded_video:
        st.info("Video processing is resource-intensive.")
            ...
            with st.spinner('Processing video...'):
                ...
                while True:
                    ...
                    if not ret:
                        break
                    results = model(frame, conf=confidence, iou=iou_threshold, verbose=False)
                    annotated, _ = annotate_image(frame, results, confidence, iou_threshold)
            ...
            for result in results:
                ...
                with col1:
                    st.image(Image.open(io.BytesIO(
                        uploaded_files[results.index(result)].read()
                    )), caption=f"Original: {result['filename']}", use_column_width=True)
                with col2:
                    annotated_rgb = cv2.cvtColor(result['image'], cv2.COLOR_BGR2RGB)
                    st.image(annotated_rgb, caption=f"Detected", use_column_width=True)
